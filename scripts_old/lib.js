...
const basedir =
  process.env.BASEDIR == undefined ? "/tmp/cannon/minime" : process.env.BASEDIR;
...
async function deploy() {
  ...
  const comp = await Computer.deploy(m.address, goldenRoot);
...
function getTrieAtStep(blockNumberN, step) {
  ...
  if (!fs.existsSync(fn)) {
    ...
    child_process.execSync(
      "mipsevm/mipsevm " + blockNumberN.toString() + " " + step.toString(),
      { stdio: "inherit" }
    );
  }
